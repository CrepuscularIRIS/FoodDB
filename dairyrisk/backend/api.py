...
from agent.modela_v2_engine import ModelAV2Engine
...
modela_v2_engine: Optional[ModelAV2Engine] = None
...
class ClosedLoopFeedbackItem(BaseModel):
    node_id: str
    label: float  # 0.0(阴性) / 1.0(阳性)


class ModelAClosedLoopRequest(BaseModel):
    budget: float = 20.0
    top_k: int = 10
    propagation_hours: int = 12
    source_node_id: Optional[str] = None
    view_mode: str = "global"  # global | product
    product_type: Optional[str] = None
    feedback: Optional[List[ClosedLoopFeedbackItem]] = None


# 初始化Agent
@app.on_event("startup")
async def startup_event():
    global agent, symptom_router, orchestrator, risk_propagator, modela_v2_engine
    ...
    print("正在初始化 ModelA v2 量化引擎...")
    modela_v2_engine = ModelAV2Engine(agent.retriever)
    print("✓ ModelA v2 量化引擎初始化完成")
...
async def modela_v2_view(
    ...
    """获取 ModeA v2 工作站初始化视图数据。"""
    if not modela_v2_engine:
        raise HTTPException(status_code=503, detail="ModelA v2 引擎未初始化")

    try:
        if refresh:
            modela_v2_engine.refresh()
        view = modela_v2_engine.build_view(
            top_n=max(1, min(top_n, 200)),
            product_type=product_type,
            max_nodes=max(100, min(max_nodes, 2500)),
            view_mode=view_mode,
        )
        return {"success": True, "data": view}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"ModelA v2 视图构建失败: {str(e)}")


@app.post("/api/modela/v2/integrated_closed_loop")
async def modela_v2_integrated_closed_loop(request: ModelAClosedLoopRequest):
    """执行 ModeA v2 风险预测 -> 抽检策略 -> 反馈回写 -> 风险传播闭环。"""
    if not modela_v2_engine:
        raise HTTPException(status_code=503, detail="ModelA v2 引擎未初始化")

    feedback = []
    for item in (request.feedback or []):
        feedback.append({"node_id": item.node_id, "label": item.label})

    try:
        closed = modela_v2_engine.run_integrated_closed_loop(
        ...
        if not source_node and closed.get("selection"):
            source_node = closed["selection"][0].get("node_id")

        propagation = None
        if risk_propagator and source_node:
            prop = risk_propagator.propagate(
                node_id=source_node,
                max_steps=max(1, min(int(request.propagation_hours), 24)),
                decay_rate=0.55,
                min_risk_threshold=0.02,
            )
            timeline = prop.get("timeline", [])
            predicted = []
            observed = []
            for frame in timeline:
                t = int(frame.get("t", 0))
                risks = [float(x.get("risk", 0.0)) for x in frame.get("nodes", [])]
                pred_val = max(risks) if risks else 0.0
                obs_val = max(0.0, min(1.0, pred_val * (0.88 + 0.1 * math.sin(t + 1))))
                predicted.append(round(pred_val, 4))
                observed.append(round(obs_val, 4))
            propagation = {
        ...
        report = modela_v2_engine.build_modea_report(closed)
        return {
            "success": True,
            "data": {
                "closed_loop": closed,
        ...
        raise HTTPException(status_code=500, detail=f"ModeA v2 闭环执行失败: {str(e)}")


@app.get("/api/modela/v2/modea_report")
async def modela_v2_report(
    budget: float = 20.0,
    top_k: int = 10,
    view_mode: str = "global",
    product_type: Optional[str] = None,
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"ModeA v2 报告生成失败: {str(e)}")


@app.get("/api/modela/v2/rolling_closed_loop")
async def modela_v2_rolling_closed_loop(sample_size: int = 100, k_eval: int = 50):
    """T1-T6 滚动闭环模拟（智能抽检 vs 随机抽检）。"""
    if not modela_v2_engine:
        raise HTTPException(status_code=503, detail="ModelA v2 引擎未初始化")
    try:
        result = modela_v2_engine.simulate_rolling_closed_loop(
            sample_size=max(20, min(sample_size, 500)),
            k_eval=max(10, min(k_eval, 500)),
        )
        return {"success": True, "data": result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"滚动闭环模拟失败: {str(e)}")
