...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            self.context_page.set_default_timeout(config.PAGE_LOAD_TIMEOUT * 1000)
            await self.context_page.goto(self.index_url, timeout=config.PAGE_LOAD_TIMEOUT * 1000)
    ...
    async def get_video_info_task(self, aid: int, bvid: str, semaphore: asyncio.Semaphore) -> Optional[Dict]:
        ...
        async with semaphore:
            if not aid and not bvid:
                utils.logger.warning("[BilibiliCrawler.get_video_info_task] Skip video detail fetch: empty aid/bvid")
                return None
            try:
    ...
    async def launch_browser(
        ...
        if config.SAVE_LOGIN_STATE:
            ...
            browser_context = await chromium.launch_persistent_context(
                ...
                channel="chromium",  # Use Playwright Chromium to avoid system Chrome dependency
            )
            return browser_context
        else:
            # type: ignore
            browser = await chromium.launch(headless=headless, proxy=playwright_proxy, channel="chromium")
