...
class RiskAssessmentAgent:
    ...
    def _calculate_graph_metrics(self) -> dict:
        ...
        try:
            import pandas as pd
            data_dir = self.retriever.data_dir

            # 读取企业数据
            ent_path = data_dir / "enterprise_master.csv"
            if ent_path.exists():
                df_ent = pd.read_csv(ent_path)
                total_nodes = len(df_ent)
                node_type_distribution = df_ent['node_type'].value_counts().to_dict() if 'node_type' in df_ent.columns else {}
            else:
                total_nodes = 0
